...
interface Question {
  sentence: string; // contains a ___ blank
  answer: string;
  options: string[];
}

const QUESTIONS: Question[] = [
  { sentence: 'She ___ to school every day.', answer: 'goes', options: ['goes', 'go', 'going', 'gone'] },
  { sentence: 'They ___ playing football now.', answer: 'are', options: ['is', 'are', 'am', 'be'] },
  { sentence: 'I have ___ apple in my bag.', answer: 'an', options: ['a', 'an', 'the', 'some'] },
  { sentence: 'He ___ his homework yesterday.', answer: 'did', options: ['do', 'does', 'did', 'done'] },
  { sentence: 'We ___ happy to see you.', answer: 'were', options: ['was', 'were', 'is', 'are'] },
  { sentence: 'The cat is ___ the table.', answer: 'under', options: ['under', 'between', 'among', 'into'] },
];
...
export const FillBlankGame: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-gradient-to-br from-primary-600 via-primary-700 to-yellow-500 text-white px-4 py-4 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/games')} className="p-1 hover:bg-white/10 rounded-lg transition-colors">
              <ChevronLeft className="w-6 h-6" />
            </button>
            <h1 className="text-xl font-bold">Fill the Blank</h1>
          </div>
              ...
              {question.options.map((option) => {
                ...
                return (
                  <button
                    ...
                    disabled={!!selected}
                    className={`p-4 rounded-xl font-semibold transition-all ${cls}`}
                  >
                    {option}
                  </button>
                );
