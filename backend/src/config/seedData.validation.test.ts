import { level1Lessons } from './seedData.js';
...
describe('seedData.ts Validation', () => {
  describe('Lesson count constraints', () => {
    ...
    test('should not have a Lesson 9 (only 1-8)', () => {
      const lesson9 = level1Lessons.find((lesson) => lesson.lessonNumber === 9);
  ...
  describe('Exercise count constraints', () => {
    test('each lesson should have exactly 5 exercises', () => {
      level1Lessons.forEach((lesson, _index) => {
  ...
  describe('Summary validation report', () => {
    test('generate validation summary', () => {
      const summary = {
        ...
        levelId: 'All 1',
        lessonNumbers: level1Lessons.map((l) => l.lessonNumber).sort((a, b) => a - b),
        exerciseCounts: level1Lessons.map((l) => l.exercises.length),
        objectiveCounts: level1Lessons.map((l) => l.content.objectives.length),
        allExerciseCountsValid: level1Lessons.every((l) => l.exercises.length >= 3 && l.exercises.length <= 20),
        allObjectiveCountsValid: level1Lessons.every(
          (l) => l.content.objectives.length >= 1 && l.content.objectives.length <= 5
        ),
      };
      ...
      expect(summary.exerciseCounts.every((count) => count === 5)).toBe(true);
      expect(summary.objectiveCounts.every((count) => count === 3)).toBe(true);
